...
function add_log($data)
{
	...
	elseif(!isset($data['date'])){$data['date'] = date("Y-m-d H:i:s"); }
...
function is_log()
{
	...
	$ci->db->where('date', $_POST['log_time']);
	if(isset($_POST['log_type'])) { $ci->db->where('type', $_POST['log_type']); } 
...
function get_log_table($data, $order_by='ASC', $array=array())
{
	...
	if(!isset($array['type'])){$array['type'] = true;}
	if(!isset($array['user'])){$array['user'] = true;}
	if(!isset($array['invoice_id'])){$array['invoice_id'] = true;}
	?>
	...
	if(isset($_POST['add_log_user_reviews']) and is_log())
	{
		...
		if(isset($data['invoice_id'])) { $log['invoice_id'] = $data['invoice_id']; }
	...
	<table class="table table-hover table-bordered table-condensed table-striped dataTable_noExcel_noLength">
            <thead>
                <tr>
                    ...
                    <?php if($array['type']==true):?><th width="80"><?php lang('Type'); ?></th><?php endif; ?>
                    <?php if($array['user']==true):?><th width="200"><?php lang('User'); ?></th><?php endif; ?>
                    <?php if($array['invoice_id']==true):?><th width="100"><?php lang('Invoice ID'); ?></th><?php endif; ?>
        ...
        <tr>
            ...
            <?php if($array['type']==true):?><td><?php echo $log['type']; ?></td><?php endif; ?>
            ...
            <?php if($array['invoice_id']==true):?><td><?php if($log['invoice_id'] > 0):?><a href="<?php echo site_url('invoice/view/'.$log['invoice_id']); ?>" target="_blank">#<?php echo $log['invoice_id']; ?></a><?php endif; ?></td><?php endif; ?>
